...
import requests
from weibo.usersettings import headers

# from utils import (code_verificate, getip)
# from weibo.error import is_403
...
def login_by_pincode(name, password, session, server_data, retry_count, proxy):
    ...
    verify_code = input("请输入" + pincode_name + "中的验证码：")
...
def login_retry(name, password, session, proxy, rs='pinerror', retry_count=0):
    while rs == 'pinerror':
        ...
        rs,  session = login_by_pincode(name, password, session, server_data, retry_count, proxy)
    return rs, session
...
def do_login(name, password, proxy):
    ...
    if server_data['showpin']:
        rs, session = login_by_pincode(name, password, session, server_data, 0, proxy)
        if rs == 'pinerror':
            rs, session = login_retry(name, password, session, proxy)

    else:
        rs, session = login_no_pincode(name, password, session, server_data, proxy)
...
def get_session(name, password):
    ...
    if url != '':
        ...
        if m and m.group(1):
            ...
            # if is_403(resp.text):
            #     print('account {} has been forbidden'.format(name))
